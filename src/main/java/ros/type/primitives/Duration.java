...
public class Duration extends TimeBase<Duration> {
    ...
    public static Duration fromJSONObject(JSONObject jsonObject) {
        int secs = jsonObject.has(Duration.FIELD_SECS) ? jsonObject.getInt(Duration.FIELD_SECS) : 0;
        int nsecs = jsonObject.has(Duration.FIELD_NSECS) ? jsonObject.getInt(Duration.FIELD_NSECS) : 0;
    ...
    public boolean sleep() {
        try {
            Thread.sleep((this.secs * TimeBase.SECS_TO_MILLI) + ((long) ((double) this.nsecs / (double) TimeBase.MILLI_TO_NSECS)), this.nsecs % (int) TimeBase.MILLI_TO_NSECS);
            return true;
